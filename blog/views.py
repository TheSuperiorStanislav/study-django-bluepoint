...
class ShareBlogPostView(TemplateView):
    ...
    def get_context_data(self, pk,**kwargs):
        ...
        shared_with = blog_post.shared_to.all()
        shared_with_ids = map(lambda x: x.pk, shared_with)
        exclude_from_can_share_list = [blog_post.blog.pk] + list(shared_with_ids)
        can_be_shared_with = Blog.objects.exclude(pk__in = exclude_from_can_share_list)
        return {
            'post': blog_post,
            'is_shared_with': shared_with,
...
class StopShareingPostWithBlog(View):
    @method_decorator(login_required)
    def dispatch(self, request, *args, **kwargs):
        return super(StopShareingPostWithBlog, self).dispatch(request, *args, **kwargs)
